...
{
    public class GetCrawlJobDataBehaviour : Chad1ProviderTest
    {
        ...
        [Theory]
        [InlineAutoData(Chad1Constants.KeyName.ApiKey, nameof(Chad1CrawlJobData.ApiKey))]
        ...
        [Theory]
        [InlineAutoData]
        public async Task Chad1CrawlJobDataReturned(Dictionary<string, object> dictionary, Guid organizationId, Guid userId, Guid providerDefinitionId)
        {
            Assert.IsType<CluedIn.Crawling.Chad1.Core.Chad1CrawlJobData>(
                await Sut.GetCrawlJobData(_context, dictionary, organizationId, userId, providerDefinitionId));
        }
    }
}
